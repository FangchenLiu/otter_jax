...
from otter.model.transformer_policy import OtterModule
from otter.utils.spec import ModuleSpec
from otter.utils.typing import Config, Data, Params, PRNGKey, Sequence
...
class OtterModel:
    ...
    Usage for inference:
        ...
        >>> model = OtterModel.load_pretrained(checkpoint_dir)
    ...
    Usage for finetuning:
        ...
        >>> model = OtterModel.load_pretrained(checkpoint_dir)
        >>> train_state = otter.utils.train_utils.TrainState.create(
    ...
    module: OtterModule = struct.field(pytree_node=False)
        ...
        tasks.update(
            {
                ...
                if k not in ("language_instruction") and k not in ("pad_mask_dict")
            }
        )
    ...
    def forward_transformer(
        ...
        Args:
            ...
            actions: dict of actions of shape (batch_size, action_horizon, action_dim)
        ...
        _verify_shapes(tasks, "tasks", self.example_batch["task"], starting_dim=1)
        _verify_shapes(actions, "actions", self.example_batch["action"], starting_dim=2)
    ...
    def get_attention(
        ...
        action, features = self.module.apply(
            {"params": self.params},
            observations,
            tasks,
            train=train,
    ...
    def load_pretrained(
        ...
        module = OtterModule.create(**config["model"])
        ...
        text_processor = ModuleSpec.create(
            text_processor_dict[config["text_processor"]]
        )
